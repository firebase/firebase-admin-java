...
public class PersistenceTestIT {
  ...
  public void persistedWritesAreUsed() {
    ...
    Map<Path, Node> merge = new HashMap<>();
  ...
  public void pruningSanityCheck() {
    ...
    TestCachePolicy cachePolicy =
        new TestCachePolicy(/*percentToPrune=*/ 1.0f, /*maxToKeep=*/ Long.MAX_VALUE);
    PersistenceManager manager =
    ...
    Map<String, Object> data =
        fromSingleQuotedString("{'deep':'foo-deep', " + "'deep2':'foo-deep2'}");
    ...
    ArrayList<QuerySpec> expectedQueries =
        new ArrayList<>(Arrays.asList(fooQuery, fooDeepQuery, barQuery, barLimitQuery, bazQuery));
    assertTrackedQueriesMatch(expectedQueries, storage.loadTrackedQueries());

    data =
        fromSingleQuotedString(
            "{'foo': {'deep': 'foo-deep', 'deep2': 'foo-deep2'}, 'bar': {'a':1, 'b':2, 'c':3, "
                + "'d':4}, 'baz': 'baz-val'}");
  ...
  public void pruningDoesOldestFirst() throws InterruptedException {
    MockPersistenceStorageEngine storage = new MockPersistenceStorageEngine();
    TestCachePolicy cachePolicy =
        new TestCachePolicy(/*percentToPrune=*/ 0.5f, /*maxToKeep=*/ Long.MAX_VALUE);
    ...
    ArrayList<QuerySpec> expectedQueries =
        new ArrayList<>(Arrays.asList(fooQuery, barQuery, bazQuery, quxQuery));
  ...
  private void assertTrackedQueriesMatch(List<QuerySpec> expected, List<TrackedQuery> actual) {
    ...
    HashSet<QuerySpec> expectedSet = new HashSet<>(expected);
  ...
  @Test
  public void revertedWriteIsDeleted() throws Throwable {
    PersistenceManager manager = newTestPersistenceManager();
    DatabaseConfig cfg = TestHelpers.newFrozenTestConfig();
    DatabaseReference ref = refWithConfig(cfg, manager);
    try {
      String noWriteRules = "{\"rules\": {\".read\": true,\".write\": false}}";
      TestHelpers.uploadRules(ref, noWriteRules);
      TestHelpers.goOffline(cfg);
      final Semaphore done = new Semaphore(0);
      ref.setValue(
          "foo-value",
      ...
      TestHelpers.waitForQueue(ref);
      assertEquals(1, manager.loadUserWrites().size());
      TestHelpers.goOnline(cfg);
      TestHelpers.waitFor(done);
      assertEquals(0, manager.loadUserWrites().size());
    } finally {
      TestHelpers.goOnline(cfg);
      TestHelpers.uploadRules(ref, TestConstants.DEFAULT_RULES_STRING);
    }
  }
  ...
  public void deepUpdateTest() throws Throwable {
    ...
    Object value = new ReadFuture(ref2).waitForLastValue();
    assertEquals(
        value, TestHelpers.fromSingleQuotedString("{ 'foo': { 'deep': { 'update': 'bar' } } }"));
  }
